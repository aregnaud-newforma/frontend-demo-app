 ...
 *
 * It mirrors the shell's main.tsx rather than being imported from it: the entry
 * builds its providers inline around <App />, so there is nothing to import.
 * The one thing to keep in step is the LIST - a provider added to main.tsx and
 * not here lands as a crash, or a blank render, in whichever test first mounts
 ...
 */
export function AppProviders({ children, app }: { children: React.ReactNode; app: AppUnderTest }) {
  const [queryClient] = useState(app.createQueryClient);
